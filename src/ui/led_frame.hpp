#ifndef led_frame_hpp_
#define led_frame_hpp_
...
public:
  ...
  [[nodiscard]] std::uint32_t& operator[](std::size_t index) { return m_FrameBuffer[index]; }
  [[nodiscard]] const std::uint32_t& operator[](std::size_t index) const { return m_FrameBuffer[index]; }
