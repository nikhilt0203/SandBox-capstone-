...
public:
  ...
  [[nodiscard]] std::optional<sndbx::grid::Position> getPosition(std::uint32_t id) const
  {
    ...
    auto it = findWithID(id);
    if (it != m_ModuleRegistry.end()) { position = (*it).position; };
    return position;
  }

  [[nodiscard]] std::optional<sndbx::grid::Position> getPosition(Module* m) const
  {
    auto it = std::find_if(
    ...
    if (it == m_ModuleRegistry.end()) { return std::nullopt; }
    return it->position;
  }
  ...
  [[nodiscard]] bool positionOccupied(sndbx::grid::Position pos) const { return findWithPosition(pos) != m_ModuleRegistry.end(); }
